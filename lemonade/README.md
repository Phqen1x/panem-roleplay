...
export from there lands back in this format.
...
`panem_bot.services.dialogue` is the first real caller of this contract (`RequestMode.DIALOGUE`
only, for now, via `/talk`) -- see the main README's Phase 6 notes for exactly what's wired up
and what (narration, broadcasts, transcription, TTS playback) is still just a documented
contract waiting for its own command/system.
...
- **For packaging**: `scripts/lemonade_omni.py bundle` produces a self-contained folder --
  ...
  the deployment-ready layout from Lemonade's guide, for whatever packaging format you ship
  Panem in. Neither `serve` nor `bundle` has been exercised in this session (no way to
  download multi-GB models here) -- treat them as implemented-per-Lemonade's-docs but
  unverified until someone runs them for real.
