"""Character lifecycle (Spec §3.1 FR-CHR). Re-exported from
`panem_bot.services.characters` for every existing call site -- moved here
(same reasoning as `panem_shared.jail`/`panem_shared.stealing`) because the
web dashboard's character endpoints (`panem_api.dashboard_routes`) need
this too and `panem_api` cannot import `panem_bot`. Nothing here has ever
depended on discord.py; this was a package-boundary accident, not a real
coupling to fix."""
