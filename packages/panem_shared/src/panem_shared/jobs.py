...
`Character.job_title` + `shift_phase` instead, see `panem_shared.shifts`
and `panem_bot.cogs.characters`); NPCs still do, so these thin lookups
stay for `residents.py`'s NPC display and `/staff job list`'s NPC-job
visibility -- and now the web dashboard's Residents tab, which is why
this moved here from `panem_bot.services.jobs` (same reasoning as every
other `panem_shared` move this session): `panem_api` cannot import
`panem_bot`. `session` is unused now that there's no more DB-backed
`JobOverride` layered on top -- kept in the signature so existing call
sites don't need to change. Re-exported from `panem_bot.services.jobs`
