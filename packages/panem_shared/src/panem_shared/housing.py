...
Pure logic, no DB session -- mirrors `panem_shared.travel`/`market`:
takes ORM/content objects as arguments, raises `NotAllowed`/`NotFound` on
refusal, leaves the actual row mutation and session handling to the
caller. Payment collection, foreclosure, and auction resolution over
time are `panem_sim.systems.housing`'s job, not this module's -- this
only covers the player-initiated actions (buy financed, refinance,
list, bid).

Moved here from `panem_bot.services.housing` (same reasoning as every
other `panem_shared` move this session) because the web dashboard's
Housing tab needs this too and `panem_api` cannot import `panem_bot`.
Re-exported from `panem_bot.services.housing` for every existing call
site.
"""
